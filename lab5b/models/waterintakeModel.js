...
const { MY_DB } = require("../config/env.js");
...
const addOne = async (waterintake) => {
  try {
    const { date, amountInLiters, comments } = waterintake;

    const sql = connection.format(
      "INSERT INTO waterintakes_table (date, amountInLiters, comments) VALUES (?, ?, ?)",
      [date, amountInLiters, comments]
    );
    const [rows] = await connection.query(sql);
